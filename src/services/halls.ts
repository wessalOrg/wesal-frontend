...
import { findHallDetailsFallback } from "@/constants/hallDetailsFallback";
import {
  REGION_API_PARAMS,
  type FeaturedHall,
  type HallAmenity,
...
function mapAvailabilityDays(
  ...
  return availability.slice(0, 7).map((day) => ({
    ...
    dateIso: day.date ? String(day.date).slice(0, 10) : undefined,
...
type ApiHallDetails = {
  ...
  shortDescription?: string | null;
  amenities?: Array<{ id?: string; label?: string; name?: string; icon?: HallAmenity["icon"] }>;
  reviews?: Array<{
    id?: string;
    author?: string;
    userName?: string;
...
/**
 * US-LAND-04 — GET /v1/halls/:id
 */
export async function fetchHallById(id: string): Promise<HallDetailsLoadResult> {
